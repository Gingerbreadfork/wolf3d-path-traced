...
static void InitGame()
{
    ...
    // (Removed legacy SDL 1.2 Win32 code that stripped the window's system menu
    // via the native HWND; the SDL2 platform layer owns the window now.)
...
void CheckParameters(int argc, char *argv[])
{
    ...
    for(int i = 1; i < argc; i++)
    {
        ...
        else IFARG("--data")
        {
            if(++i >= argc)
            {
                printf("The data option is missing the dir argument!\n");
                hasError = true;
            }
            else PLAT_SetDataDir(argv[i]);
        }
...
int main (int argc, char *argv[])
{
    ...
    // Resolve and enter the Wolfenstein data directory (--data, $WOLF3D_DATA,
    // or the bundled shareware) before any data file is touched.
    PLAT_ResolveAndEnterDataDir();
