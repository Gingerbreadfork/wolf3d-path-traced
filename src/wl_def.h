...
// SDL 1.2 -> SDL2 compatibility shims and platform-layer glue.
#include "platform/sdl_compat.h"
...
// Presentation is owned by the Vulkan platform layer. The classic frame is
// rendered into the 8-bit `screenBuffer` and presented (through the current
// palette) by PLAT_PresentClassic(). SDL_Flip maps onto that.
extern "C" void PLAT_PresentClassic(void);
#define SDL_Flip(x) PLAT_PresentClassic()

// NOTE: the original game relies on "#pragma pack(1)" (set near the top of this
// header) remaining active for the whole translation unit, including structs
// defined in the .cpp files that mirror on-disk map/save formats. We therefore
// do NOT reset packing here. Instead, renderer/platform headers that are shared
// across the game<->renderer boundary (platform.h, mixer_compat.h, rt_scene.h)
// individually guard their structs with "#pragma pack(push,8)"/"pop" so they
// keep a consistent layout regardless of include context.
